...
module Text =
    ...
    let convertToWebAssemblyText equation =
        convert equation |> Result.map (fun webAssembly -> webAssembly.Text)
    ...
    let tests =
        testList "Convert to WebAssembly text" [
            ...
            testPropertyWithConfig config "Invalid" <| fun (InvalidEquation (invalidEquation, invalidToken)) ->
                Expect.equal (convertToWebAssemblyText invalidEquation) (Error (Invalid invalidToken)) "Should be invalid"
            ...
            testPropertyWithConfig config "Single operand" <| fun (number: PositiveInt) ->
                ...
                Expect.equal (convertToWebAssemblyText (string number.Get)) (Ok (WebAssemblyText expected)) "Should convert single operand to WAT"
            ...
            testCase "All expression types" <| fun () ->
                ...
                Expect.equal (convertToWebAssemblyText equation) (Ok (WebAssemblyText expected)) "Should convert successfully to WAT"
      ]

module Binary =
    open Krakow.Core.WebAssembly.Binary
    
    let convertToWebAssemblyBinary equation =
        convert equation |> Result.map (fun webAssembly -> webAssembly.Binary)
    ...
    let tests =
        testList "Convert to WebAssembly binary" [
            ...
            testPropertyWithConfig config "Invalid" <| fun (InvalidEquation (invalidEquation, invalidToken)) ->
                Expect.equal (convertToWebAssemblyBinary invalidEquation) (Error (Invalid invalidToken)) "Should be invalid"
                
            testPropertyWithConfig config "Valid" <| fun (ValidEquation validEquation) ->
                Expect.isOk (convertToWebAssemblyBinary validEquation) "Should be valid"
                
            testPropertyWithConfig config "Single operand" <| fun () ->
                let expected =
                    [0x00; 0x61; 0x73; 0x6d; 0x01; 0x00; 0x00; 0x00; 0x01; 0x05;
                     ...
                     0x0b]
                Expect.equal (convertToWebAssemblyBinary "2") (Ok (WebAssemblyBinary expected)) "Should convert single operand to WASM"
            ...
            testCase "All expression types" <| fun () ->
                let expected =
                    [0x00; 0x61; 0x73; 0x6d; 0x01; 0x00; 0x00; 0x00; 0x01; 0x05;
                     ...
                     0x6B; 0x0B]
                let equation = "15 7 1 1 + - / 3 * 2 1 1 + + -"
                Expect.equal (convertToWebAssemblyBinary equation) (Ok (WebAssemblyBinary expected)) "Should convert successfully to WASM"
      ]