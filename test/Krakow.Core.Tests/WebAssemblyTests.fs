    ...
    let ``Convert single operand equation to WebAssembly text`` () =
        let expected = "(module " +
                         "(func (export \"evaluate\") (result i32) " +
                            "i32.const 1))"
        parse "1" |> Result.map equationToWebAssemblyText = (Ok (WebAssemblyText expected)) |> should be True
    ...
    let ``Convert single operator equation to WebAssembly text`` () =
       let expected = "(module " +
                        "(func (export \"evaluate\") (result i32) " +
                            ...
                            "i32.const 8 " +
                            "i32.sub))"
       parse "9 8 -" |> Result.map equationToWebAssemblyText = (Ok (WebAssemblyText expected)) |> should be True
    ...
    let ``Convert complex equation to WebAssembly text`` () =
        let expected = "(module " +
                        "(func (export \"evaluate\") (result i32) " +
                           ...
                           "i32.add " +
                           "i32.sub))"
        parse "15 7 1 1 + - / 3 * 2 1 1 + + -" |> Result.map equationToWebAssemblyText = (Ok (WebAssemblyText expected)) |> should be True
    ...
    let ``Convert single operand equation to WebAssembly binary`` () =
        let expected =
            [0x00; 0x61; 0x73; 0x6d; 0x01; 0x00; 0x00; 0x00; 0x01; 0x05;
             ...
             0x0b]
        parse "2" |> Result.map equationToWebAssemblyBinary = (Ok (WebAssemblyBinary expected)) |> should be True
    ...
    let ``Convert single operator equation to WebAssembly binary`` () =
       let expected =
             [0x00; 0x61; 0x73; 0x6d; 0x01; 0x00; 0x00; 0x00; 0x01; 0x05;
              ...
              0x41; 0x08; 0x6B; 0x0B]
       parse "9 8 -" |> Result.map equationToWebAssemblyBinary = (Ok (WebAssemblyBinary expected)) |> should be True
    ...
    let ``Convert complex equation to WebAssembly binary`` () =
        let expected =
              [0x00; 0x61; 0x73; 0x6d; 0x01; 0x00; 0x00; 0x00; 0x01; 0x05;
               ...
               0x6B; 0x0B]
        parse "15 7 1 1 + - / 3 * 2 1 1 + + -" |> Result.map equationToWebAssemblyBinary = (Ok (WebAssemblyBinary expected)) |> should be True